...
public:
	using value_type = std::pair<const tkey, tvalue>; //даем псевдоним для типа
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(iterator begin, iterator end, const compare& cmp,
																   pp_allocator<typename binary_search_tree<tkey, tvalue, compare, tag>::value_type> alloc, logger* logger) : compare(cmp), _allocator(alloc) {
	_root = nullptr;
	_logger = logger;
	_size = 0;
