...
private:
	...
	bool fix_underflow(btree_node* parent, size_t idx);
...
typename B_tree<tkey, tvalue, compare, t>::btree_iterator B_tree<tkey, tvalue, compare, t>::find(const tkey& key) {
	...
	std::stack<std::pair<btree_node**, size_t>> path;
	btree_node** ptr = &_root; //current pointer
	btree_node* node = *ptr; //current
...
B_tree<tkey, tvalue, compare, t>::erase(const tkey& key) {
    ...
    remove_key = [this, &remove_key](btree_node* node, const tkey& k) -> bool {
        size_t idx = 0;
        while (idx < node->_keys.size() && compare_keys(node->_keys[idx].first, k)) {
            ++idx;
        }
        ...
        if (idx < node->_keys.size() && 
            ...
            if (node->_pointers.empty()) {
                ...
                if (underflow) {
                    return this->fix_underflow(node, idx);
                }
                return false;
            }
        }
...
template<typename tkey, typename tvalue, compator<tkey> compare, std::size_t t>
bool B_tree<tkey, tvalue, compare, t>::fix_underflow(btree_node* parent, size_t idx) {
    btree_node* child = parent->_pointers[idx];
    
    if (idx > 0 && parent->_pointers[idx-1]->_keys.size() > minimum_keys_in_node) {
        btree_node* left_sib = parent->_pointers[idx-1];
        
        child->_keys.insert(child->_keys.begin(), parent->_keys[idx-1]);
        
        parent->_keys[idx-1] = left_sib->_keys.back();
        ...
        return false;
    } else if (idx < parent->_keys.size() && 
            parent->_pointers[idx+1]->_keys.size() > minimum_keys_in_node) {
        btree_node* right_sib = parent->_pointers[idx+1];
        
        child->_keys.push_back(parent->_keys[idx]);
        
        parent->_keys[idx] = right_sib->_keys.front();
        right_sib->_keys.erase(right_sib->_keys.begin());
        
        if (!right_sib->_pointers.empty()) {
            child->_pointers.push_back(right_sib->_pointers.front());
        ...
        if (idx > 0) {
            ...
            left_sib->_keys.push_back(parent->_keys[idx-1]);
            
            left_sib->_keys.insert(left_sib->_keys.end(),
                                 child->_keys.begin(),
                                 child->_keys.end());
            left_sib->_pointers.insert(left_sib->_pointers.end(),
                                     child->_pointers.begin(),
                                     child->_pointers.end());
            
            parent->_keys.erase(parent->_keys.begin() + idx - 1);
            parent->_pointers.erase(parent->_pointers.begin() + idx);
            delete child;
        } else {
            btree_node* right_sib = parent->_pointers[idx+1];
            ...
            child->_keys.insert(child->_keys.end(),
                              right_sib->_keys.begin(),
                              right_sib->_keys.end());
            child->_pointers.insert(child->_pointers.end(),
                                  right_sib->_pointers.begin(),
                                  right_sib->_pointers.end());
